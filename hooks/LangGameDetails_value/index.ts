...
export const useLangGameDetails = () => {
  ...
  const autoGetData = useCallback((lang: Locale, slug: string) => {
    ...
    const getGameDetailsFromCache = useCallback((lang: Locale, name: string) => {
      ...
      const blogDetailsForLang = langGameDataState[lang as keyof typeof langGameDataState];
  
      if (!blogDetailsForLang) {
        return null;
      }
  
      return blogDetailsForLang.find((item: GameDetails) => item.name === name);
    }, [langGameDataState]); 
  // 返回由 useState 管理的本地状态变量
  return { LangGameDetails: langGameDataState, updateLanguage, updataLangGameDetailsByLang, clearLanguageData, autoGetData ,getGameDetailsFromCache}
}